...
def make_payment_request(**args):
	...
	if dt not in [
		...
		frappe.throw(_("Payment Requests cannot be created against: {0}").format(frappe.bold(dt)))
	...
	if args.loyalty_points and ref_doc.doctype == "Sales Order":
		...
		# If this is called during save on the refdoc, this method doesn't work and will
		# be overridden in during the transaction
		if not args.ref_doc:
			frappe.db.set_value(
				ref_doc.doctaype,
				ref_doc.name,
				{
					"loyalty_points": int(args.loyalty_points),
